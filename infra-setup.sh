...
TEST_REDIS_SERVER_NAME='monetary_test_redis'
TEST_REDIS_IMAGE='bitnami/redis:latest'
TEST_REDIS_ENVS='REDIS_PASSWORD=redisPassword'
TEST_REDIS_PORT=6379

TEST_KAFKA_NAME='monetary_test_kafka'
TEST_KAFKA_ENVS='--env KAFKA_BROKER_ID=1 --env KAFKA_ZOOKEEPER_CONNECT='monetary_test_zookeeper:2181' --env KAFKA_ADVERTISED_LISTENERS='PLAINTEXT://localhost:9092' --env KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR=1 --env KAFKA_GROUP_INITIAL_REBALANCE_DELAY_MS=0 --env KAFKA_AUTO_CREATE_TOPICS_ENABLE=true'
TEST_KAFKA_DATA='monetary_test_kafka_data'
KAFKA_DATA_PATH='var/lib/kafka/data'

TEST_ZOOKEEPER_NAME='monetary_test_zookeeper'
...
up_db() {
    if [[ ! $(isUp 5433) ]]; then
        ...
        docker run --network ${TEST_NETWORK} --name ${TEST_DB_NAME} --mount source=${TEST_DB_DATA},target=/var/lib/postgresql -e POSTGRES_PASSWORD=${TEST_DB_PASSWORD} -e POSTGRES_DB={TEST_DB_NAME} -e POSTGRES_USER=${TEST_DB_USERNAME} -p ${TEST_DB_PORT}:5432 postgres
        isDBUp
    else
